...
void run_demo(BuddyAllocator &alloc, VMManager &vm, CacheHierarchy &cache) {
    ...
    print_info("Reading the same pages again to demonstrate TLB and cache locality...");
...
int main(){
    ...
    CacheLevelConfig l3{512*1024,64,16,ReplacementPolicy::LRU,WritePolicy::WriteBack,AllocationPolicy::WriteAllocate,12};
    ...
    while(getline(cin,line)){
        ...
        else if(cmd=="read"){int pid;uint64_t va;if(!(iss>>pid>>hex>>va)){print_error("Usage: read <pid> <va_hex>");continue;}auto pa=vm.translate(pid,va);if(!pa){print_error("Translation failed.");continue;}uint8_t val;if(cache.read(*pa,&val,1)){ostringstream o;o<<"Read [PID "<<pid<<" VA 0x"<<hex<<va<<"] -> "<<dec<<(int)val;print_success(o.str());}else print_error("Memory read failed.");}
        else if(cmd=="write"){int pid;uint64_t va;int v;if(!(iss>>pid>>hex>>va>>v)){print_error("Usage: write <pid> <va_hex> <value>");continue;}auto pa=vm.translate(pid,va);if(!pa){print_error("Translation failed.");continue;}uint8_t val=(uint8_t)v;if(cache.write(*pa,&val,1)){ostringstream o;o<<"Wrote "<<v<<" to [PID "<<pid<<" VA 0x"<<hex<<va<<"]";print_success(o.str());}else print_error("Memory write failed.");}
