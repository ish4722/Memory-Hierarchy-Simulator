...
{
public:
    VMManager(PhysicalMemory &pm, Allocator &alloc, size_t page_size = 4096);
    ...
    struct ProcPT
    {
        std::unordered_map<uint64_t, size_t> page_table;
        std::deque<uint64_t> clock_order;
        std::unordered_map<uint64_t, bool> ref_bits;
    };
