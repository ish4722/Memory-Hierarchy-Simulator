...
struct CacheLevelConfig {
    ...
    AllocationPolicy allocation = AllocationPolicy::WriteAllocate;
    size_t hit_latency = 1;
};
...
public:
    CacheHierarchy(PhysicalMemory &pm,
                   ...
                   CacheLevelConfig l3 = {2*1024*1024,64,16,ReplacementPolicy::LRU,WritePolicy::WriteThrough,AllocationPolicy::WriteAllocate,12});
...
private:
    ...
    public:
        ...
        bool evict_line(size_t addr, Line &out);
    ...
    bool read_block(size_t addr, std::vector<uint8_t> &data, size_t &latency);
    bool write_level(size_t level, size_t addr, const void *src, size_t len);
    bool propagate_eviction(size_t level, size_t block, const Line &line);
